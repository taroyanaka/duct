...
function allowOrigin(res){
  ...
  // all_validation_checking_client_server_bothという名前のobjectを作成して、その中に、それぞれのvalidation_checkingの関数を入れていく
          ...
          const error_check_insert_tag = (tag) => {
              ...
              switch (true) {
                  ...
                  case tag.length > 24: return '24文字以上はエラー'; break;
  ...
  function db_init2(DB) {
      ...
      const CHECK_DB_RES = DB.name === './.data/duct_test.sqlite3' ? 'OK' : 'ERROR';
  ...
  const db = test_mode() === true ? new sqlite('./.data/duct_test.sqlite3') : new sqlite('./.data/duct.sqlite3');
  ...
  app.get('/', (req, res) => {
      console.log('Hello World, this is the TEST mode!!!! on duct 2024 05 14');
      res.json({message: 'Hello World, this is the TEST mode!!!! on duct 2024 05 14'});
  });
